...
const userAgents = JSON.parse(readFileSync(`../../lib/useragents.json`, "utf-8"));
c { rdn, tensor, mm } = require("./src/utils");
...
const hcaptcha = async (page) => {
  ...
  const token = await page.evaluate(async () => {
    ...
    return await solveCaptcha(urlParams.get("sitekey"), urlParams.get("host"));
  });
